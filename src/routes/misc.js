...
router.get('/leave-types', requireAuth, async (req, res, next) => {
  try {
    res.json(await all('SELECT code,label,default_total,gender_restricted FROM leave_types'));
  } catch (e) { next(e); }
});
...
router.get('/export/requests.csv', requireAuth, requireRole('hr'), async (req, res, next) => {
  try {
    ...
    const lines = [header.join(',')].concat(
      rows.map(r => [r.name, r.dept, r.type_label, r.start_date, r.end_date, r.days, r.status, r.reason || '', r.applied_at].map(csvEscape).join(','))
    );
...
router.get('/export/employees.csv', requireAuth, requireRole('hr'), async (req, res, next) => {
  try {
    const emps = await all('SELECT id,name,dept,role,gender FROM employees ORDER BY name');
    const types = await all('SELECT code,label FROM leave_types');
    const header = ['Name', 'Department', 'Role', 'Gender', ...types.map(t => t.label + ' left')];
    ...
    for (const e of emps) {
      ...
      lines.push([e.name, e.dept, e.role, e.gender, ...types.map(t => byCode[t.code] ?? '')].map(csvEscape).join(','));
    }
    res.setHeader('Content-Type', 'text/csv');
